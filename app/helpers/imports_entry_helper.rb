...
module ImportsEntryHelper
  def options_for_mapping_select(import, field, options={})
	...
	import.columns_options.each do |item|
		if (item[0] == field)
			selected = item[1]
			break
		end
    ...
    tags << options_for_select(import.columns_options, selected)
    if values = options[:values]
      tags << content_tag('option', '--', :disabled => true)
      tags << options_for_select(values.map {|text, value| [text, "value:#{value}"]}, selected)
    end
    tags
  end
